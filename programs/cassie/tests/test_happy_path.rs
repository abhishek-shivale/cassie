...
fn account_exists(svm: &LiteSVM, addr: &Pubkey) -> bool {
    svm.get_account(addr).map(|a| a.lamports > 0).unwrap_or(false)
}
...
fn test_happy_path() -> Result<()> {
    ...
    assert_eq!(cfg.default_answer_window, 3600, "answer window should be 3600");
    assert_eq!(cfg.default_dispute_window, 7200, "initial dispute window should be 7200");
    assert_eq!(cfg.default_council_window, SECONDS_PER_DAY, "council window should be 86400");
    assert_eq!(cfg.min_bounty, 10, "min bounty should be 10");
    assert_eq!(cfg.min_stake, MIN_STAKE, "min stake should be 750");
    assert_eq!(cfg.min_dispute_bond, MIN_STAKE, "min dispute bond should be 750");
    assert_eq!(cfg.slash_bps, SLASH_BPS, "slash bps should be 5000");
    assert_eq!(cfg.treasury_bps, TREASURY_BPS, "treasury bps should be 1000");
    ...
    assert_eq!(cfg.council[..9], council[..9], "council members should match");

    // ===================== Update Config =====================
    update_config(&mut svm, &owner);

    let cfg: OracleConfig = get_account_data(&svm, &config_pda);
    assert_eq!(cfg.default_dispute_window, 7250, "dispute window should be updated to 7250");
    assert_eq!(cfg.default_answer_window, 3600, "answer window should be unchanged");
    assert_eq!(cfg.default_council_window, SECONDS_PER_DAY, "council window should be unchanged");
    ...
    let cfg: OracleConfig = get_account_data(&svm, &config_pda);
    assert_eq!(
        cfg.council[0], new_council_member.pubkey(),
        "first council slot should be replaced"
    );
    assert_eq!(cfg.council[1], council[1], "second council slot should be unchanged");
    assert!(!cfg.council.contains(&old_member), "old member should no longer be in council");
    ...
    assert_eq!(q.created_at, initial_time, "created_at should be initial time");
    assert_eq!(q.answer_deadline, initial_time + 3600, "answer deadline should be created + 3600");
    ...
    assert_eq!(token_balance(&svm, &asker_ata), 70_000 - 70, "asker should have 69_930 after bounty");

    // ===================== Propose Answer =====================
    let proposer = &get_new_account(&mut svm);
    let proposer_ata = ata(proposer.pubkey(), USDC_PUBKEY);
    let answer_pda = get_pda(&[ANSWER_SEED.as_ref(), hash.as_ref(), proposer.pubkey().as_ref()]);
    let proposer_rep_pda = get_pda(&[REPUTATION_SEED.as_ref(), proposer.pubkey().as_ref()]);
    propose_answer(&mut svm, proposer, hash, true);

    let q: Question = get_account_data(&svm, &question_pda);
    assert_eq!(q.state, QuestionState::Answering, "state should be Answering after proposal");
    ...
    assert_eq!(rep.voter, proposer.pubkey(), "reputation voter should be proposer");
    assert_eq!(rep.score, 0, "initial score should be 0");
    assert_eq!(rep.answered, 0, "initial answered should be 0");

    assert_eq!(token_balance(&svm, &proposer_ata), 1_000_000 - 750, "proposer should have 999_250 after stake");
    ...
    let q: Question = get_account_data(&svm, &question_pda);
    assert_eq!(q.state, QuestionState::Resolved, "state should be Resolved after close");
    assert!(q.answer_deadline <= initial_time + 3610, "answer deadline should be in the past");

    let o: Outcome = get_account_data(&svm, &outcome_pda);
    assert!(o.result, "result should be true (yes won)");
    assert_eq!(o.resolver, Resolver::Optimistic, "should be optimistic resolution");
    ...
    let q: Question = get_account_data(&svm, &question_pda);
    assert_eq!(q.state, QuestionState::Escalated, "state should be Escalated after dispute");
    ...
    assert!(!dc.claimed_outcome, "claimed_outcome should be false (opposite of outcome)");
    ...
    assert_eq!(ct.opened_at, initial_time + 3610, "council should have been opened at close_proposer time");
    assert_eq!(ct.total_yes_stake, 8 * 750, "total yes stake should be 8 * 750 = 6000");
    assert_eq!(ct.total_no_stake, 750, "total no stake should be 750");
    assert!(ct.finalized.is_none(), "should not be finalized yet");

    let q: Question = get_account_data(&svm, &question_pda);
    assert_eq!(q.state, QuestionState::Council, "state should be Council after voting");
    ...
    let ct: CouncilTotal = get_account_data(&svm, &council_total_pda);
    assert_eq!(ct.finalized, Some(true), "council should be finalized with verdict true");
    assert_eq!(ct.yes_count, 8, "yes count unchanged after finalize");
    assert_eq!(ct.no_count, 1, "no count unchanged after finalize");

    let q: Question = get_account_data(&svm, &question_pda);
    assert_eq!(q.state, QuestionState::Resolved, "state should be Resolved after council finalize");

    let o: Outcome = get_account_data(&svm, &outcome_pda);
    assert!(o.result, "outcome result should be true (council majority yes)");
    ...
    let q: Question = get_account_data(&svm, &question_pda);
    assert_eq!(q.state, QuestionState::Settled, "state should be Settled after settle");
    ...
    let dc: DisputeConfig = get_account_data(&svm, &dispute_pda);
    assert!(!dc.resolved, "dispute should be resolved=false (disputer was wrong)");
    ...
    assert!(!a_pre.claimed, "answer should not be claimed yet before claim");
    ...
    assert!(rep.active_days >= 1, "proposer should have at least 1 active day");
    assert!(!account_exists(&svm, &answer_pda), "answer account should be closed after claim");
    ...
    assert!(!dc_pre.resolved, "dispute should be unresolved (disputer was wrong)");
    ...
    assert!(!account_exists(&svm, &dispute_pda), "dispute account should be closed after claim");
    ...
    assert!(!account_exists(&svm, &pool_ata), "pool ATA should be closed");
    assert!(!account_exists(&svm, &question_pda), "question account should be closed");
    assert!(!account_exists(&svm, &outcome_pda), "outcome account should be closed");
