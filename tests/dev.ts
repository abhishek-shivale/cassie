...
describe("cassie [devnet]", function () {
  ...
  describe("lifecycle (optimistic resolve)", () => {
    ...
    before(async function () {
      if (!adminHasUsdc) {
        console.warn("    [skip] admin holds no USDC — cannot fund participants");
        this.skip();
      }
      // Shrink global windows so the deadlines elapse in seconds, not hours.
      await shrinkWindows(admin.publicKey);
      await provision(creator);
      await provision(proposer);
      await fundSol(cranker.publicKey, SUBACCOUNT_SOL); // cranker pays fees only
